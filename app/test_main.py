...
class TestCanAccessGooglePage(unittest.TestCase):
    ...
    def test_accessible_when_internet_and_valid_url(self) -> None:
        result = can_access_google_page("http://www.google.com")
        self.assertEqual(result, "Accessible")

    @patch("app.main.has_internet_connection", return_value=False)
    @patch("app.main.valid_google_url", return_value=True)
    def test_not_accessible_when_no_internet(self) -> None:
        result = can_access_google_page("http://www.google.com")
        self.assertEqual(result, "Not accessible")

    @patch("app.main.has_internet_connection", return_value=True)
    @patch("app.main.valid_google_url", return_value=False)
    def test_not_accessible_when_invalid_url(self) -> None:
        result = can_access_google_page("http://invalid-url.com")
        self.assertEqual(result, "Not accessible")

    @patch("app.main.has_internet_connection", return_value=False)
    @patch("app.main.valid_google_url", return_value=False)
    def test_not_accessible_when_no_internet_and_invalid_url(self) -> None:
        result = can_access_google_page("http://invalid-url.com")
        self.assertEqual(result, "Not accessible")
