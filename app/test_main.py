...
def test_accessible_when_both_true(monkeypatch: MonkeyPatch) -> None:
...
def test_not_accessible_if_only_connection_true(
...
def test_not_accessible_if_only_valid_url_true(
        monkeypatch: MonkeyPatch
) -> None:
    monkeypatch.setattr(main, "valid_google_url", Mock(return_value=True))
    monkeypatch.setattr(main,
...
def test_not_accessible_if_both_false(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(main, "valid_google_url", Mock(return_value=False))
    monkeypatch.setattr(main,
                        "has_internet_connection",
                        Mock(return_value=False)
